...
class GSBA_coverage_11(method.Method):
    ...
    ''' 
       将覆盖率以及epa作为每个节点的值，然后再进行likehood。
    ...
    def detect(self):
        ...
        for node in infected_nodes:
            w[node] =float(self.prior[node]* coverage_centralities[node] *epa_weight_cnetralities[node])
        ...
        for v in infected_nodes:
            ...
            neighbours.add(v)
            likelihood = 1
           # effective propagation probabilities: node->w
            w_key_sorted = blist()
            ...
            while len(included) < n:
                ...
                for h in new:
                    ...
                    if h in infected_nodes:
                        ...
                        # w_key_sorted[k:k] = [h]
            posterior[v] = (decimal.Decimal(likelihood) )
            print('两百个点的时候会出现什么？')
            print(posterior[v])
...
if __name__ == "__main__":
    ...
    methods = [GSBA_coverage_11(prior_detector1)]
