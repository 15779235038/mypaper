...
class FindSource:
    ...
    def cal_reverse_algorithm(self, infectG):
        ...
        for index in range(len(self.single_best_result[0])):
            source = self.revsitionAlgorithm(self.single_best_result[0][index], self.single_best_result[1], infectG,
                                             self.tempGraph)
    ...
    def cal_BFS_monte_Carlo(self,dir):
        ...
        for h in range(radius_graph // 2, radius_graph-1, 1):
    ...
    def get_center(self,tempGraph):
        # 介数中心性
        between_dict = nx.closeness_centrality(tempGraph)
