...
def contractSource(G, sourceNum, sourceMaxDistance):
    ...
    while (flag == 0):
        ...
        if sourceNum == 1:
            ...
            # random_RumorSource = random_Rumo[0]
            # print('四源点')
            # edges=list(nx.dfs_edges(G, source=random_RumorSource, depth_limit=7))   #这是从54节点开始的深度树，那么现在就是4源点实验了。取0，3，6，9这些边首节点
            # # print (edges)   #这里不能重复的。
            # flagsorce=0
            # begin=0
            # while flagsorce==0:
            #     number=0
            #     for i in range(begin,150, 4):
            #         number=number+1
            #         if number!=5:
            #             rumorSourceList.append(edges[i][0])
            #             # print (rumorSourceList)
            #         else:
            #             break
            #
            #     #判断是否重复
    ...
    for sourceNum in range(4, 5):
            ...
            if sourceNum == 1:  # 单源点。
                ...
                for i in range(1, 4):
                    ...
                    for sourcesi in range(len(Sampleset)):
                        print('当前输入list' + str(Sampleset[sourcesi]))
                        mincover = getSimilir(Sampleset[sourcesi][0], Sampleset[sourcesi][1], singleRegionList, infectionG)
                        # 往后5个位置找一个比它更好地点。只要找更好就行,找不到就返回不变就可以
                        # 当前的下标
                        currentindex =  sourceAndH.index([Sampleset[sourcesi][0], Sampleset[sourcesi][1]])
                        length = len( sourceAndH)
...
if __name__ == '__main__':
    ...
    # 构建图，这个图是有有效距离的。
    G = ContractDict('../data/Brightkite_edges.txt', Ginti)
