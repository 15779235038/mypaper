        ...
        for time in range(0,3):
             ...
             for node in infectList:
                for height in list(G.neighbors(node)):
                        ...
                        if disEffectDistance(G.edge[node][height]['weight'])>randnum:
                            G.node[height]['SI'] = 2
...
def findmultiplesource(singleRegionList, infectionG, trueSourcelist,sourceNum):
    ...
    if sourceNum == 1:  # 单源点。
      ...
      for  l  in range(0,4):
          ...
          randomSource=[random.choice(Alternativenodeset), random.choice(Alternativenodeset)]
          sourcePartition1=[]
          sourcePartition2=[]
          sourcePartition1.append(randomSource[0])
          sourcePartition2.append(randomSource[1])
          Alternativenodeset.remove( randomSource[0])
          Alternativenodeset.remove(randomSource[1])
          for node in Alternativenodeset:
              #分别计算到两个源的距离。
              length1=nx.shortest_path_length(infectionG,source=node,target=randomSource[0])
              length2=nx.shortest_path_length(infectionG,source=node,target=randomSource[1])
              if length1>length2:
                sourcePartition2.append(node)
              else:
                sourcePartition1.append(node)


            #ok,接下来已经分割完毕了。sourcePartion1，2就是我们的结果了.在这两个分区中寻找新的点，让目标函数成立。

          for  partion1node in sourcePartition1:
...
def multiplePartion(mutiplelist, infectionG, rumorSourceList):
    ...
    for sigleReionlist in mutiplelist:
        allsigleSourceList.append(findmultiplesource(sigleReionlist, infectionG, rumorSourceList,2))
